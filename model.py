...
from encoder import MambaPulseEncoder
...
class MQIScaleBranch(nn.Module):
    def __init__(self, d_model: int, num_queries: int = 400,
        ...
        assert side * side == num_queries, \
            f"num_queries must be a perfect square for 2D alignment, " \
            f"got {num_queries}"
    ...
    def align_to_grid_2d(self, feat, in_hw):
        B, L, D = feat.shape
        in_H, in_W = in_hw
        assert L == in_H * in_W
    ...
    def forward(self, queries, feat, feat_hw, num_groups: int = 1):
        B, total_Nq, D = queries.shape
...
class MQI(nn.Module):
    ...
    def forward(self, queries, memories, pos_embeds, num_groups: int = 1):
        residual = queries
...
class MambaPulseDecoder(nn.Module):
    def __init__(
        self,
        d_model: int = 256,
        num_queries: int = 400,
        ...
        side = int(round(math.sqrt(num_queries)))
        assert side * side == num_queries, \
            f"num_queries must be a perfect square (for 2D-aware MQI alignment), " \
            f"got {num_queries}. Recommended: 400 (=20^2)."
...
class MambaPulse(nn.Module):
    def __init__(
        self,
        img_size=640, patch_size=16,
        vim_depth=24,
        vim_embed_dim=192,
        ...
        super().__init__()
        self.encoder = MambaPulseEncoder(
            ...
            use_fpn=use_fpn,
            use_hires_mamba=use_hires_mamba,
            hires_d_model=hires_d_model,
            hires_num_layers=hires_num_layers,
            hires_drop_path=hires_drop_path,
            skip_p2=skip_p2,
        )
        ...
        self.decoder = MambaPulseDecoder(
